...
function command.run(message, mt)
  ...
  if cmember:hasRole(privatestuff.modroleid) then
    ...
    --0: pre-s7
    --1: s7 intro, The cat poster can be pulled away.
    --2: the cat poster has been pulled away and relocated to reveal the scanner, the terminal is waiting to be pulled from (by kin).
    --3: key card pulled by kin
    --4: key card has been used with scanner by kin. anyone can now obtain a key card, not that it is needed to access anything anymore.
    ...
    --5: someone has flipped a coin in the hallway, and the Shady Casino can be entered:
    --6: The shady casino has been entered for the first time.
