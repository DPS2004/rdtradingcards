...
--          dpf.savejson("savedata/events.json",tf)
  ...
  if uj.room == 5 then     --------------------------------------------------SHOP--------------------------------------------------------------------------   
    ...
    if (request == "shop" or request == "quaintshop" or request == "quaint shop" or request == "")  then 
      ...
      local imagemessage = message.channel:send{file = getshopimage()} -- fuck this line of code. i fucking hate this so much
      local imageurl = imagemessage.attachment.url
      imagemessage:delete() -- why do i need to send and delete a message just to get an attachment url. why.
      message.channel:send{embed = { --"todo: make this an embed"? well i did make this an embed and it's horrible
        color = 0x85c5ff,
