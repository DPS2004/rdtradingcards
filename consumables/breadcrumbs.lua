...
function item.run(uj, ujf, message, mt, interaction)
  ...
  if uj.conspt == "none" then
    ...
    interaction:reply("You eat all of the **Breadcrumbs** yourself. Surely this will lure in some birds into your location.")
	local randtime = math.random(4,8)
	uj.lastpull = uj.lastpull - randtime
	interaction:reply('Also, your pull cooldown was decreased by '..randtime..' hours!')
    dpf.savejson(ujf, uj)
  else
    interaction:reply("You already have a pull affecting item in use! You decide against using the **Breadcrumbs** for now.")
  end
end
return item