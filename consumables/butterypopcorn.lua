...
function item.run(uj, ujf, message, mt, interaction)
  ...
  if uj.conspt == "none" then
    ...
    uj.conspt = "filmreel"
    interaction:reply("You shovel the **Buttery Popcorn** into your mouth. This will surely attract some animated cards!")
	local randtime = math.random(4,8)
	uj.lastpull = uj.lastpull - randtime
	interaction:reply('Also, your pull cooldown was decreased by '..randtime..' hours!')
    dpf.savejson(ujf, uj)
  else
    interaction:reply("You already have a pull affecting item in use! You decide against eating the **Buttery Popcorn** at this time.")
  end
end
return item