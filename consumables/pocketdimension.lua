...
function item.run(uj, ujf, message, mt, interaction)
  ...
  if uj.conspt == "none" then
    ...
    interaction:reply("You smash your **Pocket Dimension** on the floor. You immediately regret it, but at least the next card you pull is guaranteed to be an Alternate one.")
	local randtime = math.random(4,8)
	uj.lastpull = uj.lastpull - randtime
	interaction:reply('Also, your pull cooldown was decreased by '..randtime..' hours!')
    dpf.savejson(ujf, uj)
  else
    interaction:reply("You already have a pull affecting item in use! You decide against using the **Pocket Dimension** at this time.")
  end
end
return item