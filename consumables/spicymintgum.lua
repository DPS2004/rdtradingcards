...
function item.run(uj, ujf, message, mt, interaction)
  ...
  if uj.conspt == "none" then
    ...
    interaction:reply("You unwrap the **Spicy Mint Gum** and start chewing on a stick. This will surely attract some ADOFAI cards!")
	local randtime = math.random(4,8)
	uj.lastpull = uj.lastpull - randtime
	interaction:reply('Also, your pull cooldown was decreased by '..randtime..' hours!')
    dpf.savejson(ujf, uj)
  else
    interaction:reply("You already have a pull affecting item in use! You decide against chewing on the **Spicy Mint Gum** at this time.")
  end
end
return item